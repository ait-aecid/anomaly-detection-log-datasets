...
parser.add_argument("--data_dir", default="hdfs_xu", help="path to input files", type=str, choices=['adfa_verazuo', 'hdfs_xu', 'hdfs_loghub', 'bgl_loghub', 'bgl_cfdr', 'openstack_loghub', 'hadoop_loghub', 'thunderbird_cfdr'])
    ...
    with open(source + '/parsed.csv') as extracted, open(source + '/' + source.split('_')[0] + '_train', 'w+') as train, open(source + '/' + source.split('_')[0] + '_test_normal', 'w+') as test_norm, open(source + '/' + source.split('_')[0] + '_test_abnormal', 'w+') as test_abnormal:
        ...
        for line in extracted:
            ...
            if cnt % 1000000 == 0:
                ...
                print(str(cnt) + ' lines processed, ' + str(len(sequences_extracted['Normal'])) + ' normal and ' + str(num_seq_anom) + ' anomalous sequences found so far')
            parts = line.strip('\n').split(';')
        ...
        for label, seq_id_dict in sequences_extracted.items():
            ...
            if label == 'Normal':
                ...
                with open(source + '_test_abnormal_' + label, 'w+') as test_label:
                    for seq_id, event_list in seq_id_dict.items():
